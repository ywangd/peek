...
def test_formatting_looped_api_calls(parser):
    nodes = parser.parse(
        '''for i in range(@foo @bar 1 10 a=4 b=2) {
  POST test-1/_doc
  { "tag": i }
  POST test-2/_doc
  { "tag": i + 1}
...
'''
    ...
    assert (
        FormattingVisitor(pretty=True).visit(nodes[0])
        == '''for i in range(@foo, @bar, 1, 10, a=4, b=2) {
  POST test-1/_doc
  {
    "tag": i
  ...
  POST test-2/_doc
  {
    ...
    assert (
        FormattingVisitor(pretty=False).visit(nodes[0])
        == '''for i in range(@foo,@bar,1,10,a=4,b=2) {
  POST test-1/_doc
  {"tag":i}
...
'''
    ...
    assert (
        TreeFormattingVisitor().visit(nodes[0])
        == '''EsApiStmt
  PUT
  _license
  Dict
...
def test_tree_formatting(parser):
    ...
    nodes = parser.parse(
        '''f @abc 1 "a" b=a foo="bar" 1 * 2 + (3-2) * 5 / 6
get ("hello" + 42 + "world")'''
    )
    assert (
        visitor.visit(nodes[0])
